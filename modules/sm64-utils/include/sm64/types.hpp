#ifndef SM64_TYPES_HPP_INCLUDED
...
namespace sm64 {
  ...
  using Vec2f = std::array<f32, 2>;
  using Vec3f = std::array<f32, 3>;  // X, Y, Z, where Y is up
  using Vec3s = std::array<s16, 3>;
  using Vec3i = std::array<s32, 3>;
  using Vec4f = std::array<f32, 4>;
  using Vec4s = std::array<s16, 4>;
